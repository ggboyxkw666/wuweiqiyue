...
function beginRound() {
  ...
  stageTitle.innerHTML = `${round.name} <span>${state.roundIndex === 0 ? '速度' : state.roundIndex === 1 ? '控制' : '稳定'}</span>`; stageDescription.textContent = `${round.description} · ${difficultyProfiles[state.difficulty].label}难度`; roundTag.textContent = round.tag; roundInstruction.textContent = round.instruction; mouseHint.querySelector('span:last-child').textContent = round.type === 'track' ? '按住左键，跟住变速目标' : (round.decoys ? '避开橙色诱导点' : '保持自然，不要刻意修正'); timerDisplay.textContent = formatTime(round.limit); updateMetrics(); clearRoundTimer(); state.roundTimer = window.setInterval(() => updateTimer(performance.now()), 80);
}
